...
app.listen(port, hostname, () => {
    console.log(`IntegrationOS Oauth Server is running on http://${hostname}:${port}`);
});

app.route("/oauth/:platform/init")
    .get(async (req, res) => {
        res.send(`To Init OAuth for Platform: <b>${await toCamelCase(req.params.platform)}</b>, perform a <b>POST</b> request!`);
    })
    .post(async (req, res) => {
        try {
            ...
            if (!(await checkExistence(platformOAuthPath))) {
                return res.status(404).send({ message: `Error: OAuth does not exist for ${platform}!` });
            }

            const { init } = require(`${platformOAuthPath}/init`);

            if (typeof init !== "function") {
                return res.status(500).send({ message: `Error: Missing init function for ${platform}!` });
            }
...
app.route("/oauth/:platform/refresh")
    .get(async (req, res) => {
        res.send(`To Refresh OAuth for Platform: <b>${await toCamelCase(req.params.platform)}</b>, perform a <b>POST</b> request!`);
    })
    .post(async (req, res) => {
        try {
            const { platform } = req.params;
            ...
            if (typeof refresh !== "function") {
                return res.status(500).send({ message: `Error: Missing refresh function for ${platform}!` });
            }
